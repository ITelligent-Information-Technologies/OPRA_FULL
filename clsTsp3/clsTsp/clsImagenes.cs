...
namespace clsTsp
{
    class clsImagenes
    {
        ...
        public double PintarRecorrido(List<clsPunto> lstPuntos, Int32 intWidth, Int32 intHeight, string strFilePathOutput, Boolean blnPintarSoloCiudades)
        {
            double dblCoste = 0;
            Bitmap bm = new Bitmap(intWidth, intHeight);
            Image img = new Bitmap(bm);
            ...
            for (Int32 intI = 1; intI < lstPuntos.Count; intI++)
            {
                float dblOldX = lstPuntos[intI - 1].intX;
                float dblOldY = lstPuntos[intI - 1].intY;
                float dblX = lstPuntos[intI].intX;
                float dblY = lstPuntos[intI].intY;
                Rectangle rec = new Rectangle(Convert.ToInt32(dblX - 3), Convert.ToInt32(dblY - 3), 6, 6);
                drawing.FillRectangle(Brushes.DarkGray, rec);
                if (intI == 1)
                {
