...
namespace clsReinforcementLearning
{
    /// <summary>
    /// Ejemplo realizado de Qlearning para probar y ver resultados.
    /// en este caso sobre un ejemplo de juguete y sin que vaya a servir
    /// para OPRA ya que no es nada de deep learning
    /// </summary>
    class QLearningProgram
    {
        ...
        static void Print(double[][] Q)
        {
            ...
            Console.Write("    [5]    [6]    [7]    [8]    [9]");
            Console.WriteLine("   [10]   [11]");
            for (int i = 0; i < ns; ++i)
            {
                string row = "[" + i.ToString().PadLeft(2) + "]";
                Console.Write(row);
                for (int j = 0; j < ns; ++j)
                {
                    string s = Q[i][j].ToString("F2");
                    if (s.StartsWith("-") == false)
                    {
                        Console.Write(" "); Console.Write(s.PadLeft(6));
                    }
                    else
                        Console.Write(s.PadLeft(7));
                }
                Console.WriteLine();
            }
        }
