...
namespace clsTsp
{
    ...
    class clsSA
    {
        ...
        public void Optimizar(List<clsPunto> lstPuntos, string strPathFileIn, string strPathFileOut)
        {
            ...
            cImg.PintarRecorrido(lstPuntos, lstRecorrido, 500, 500, @"d:\borrar\fig1.jpg", false);
            Boolean blnEnBucle = true;

            Boolean blnHaQuitadoCruce = true;
            Boolean blnDirecto = true;
            while(blnHaQuitadoCruce)
            (dblCoste, blnHaQuitadoCruce) = QuitarTodosLosCruces(ref lstPuntos);
            string strLineas = "";
            cImg.PintarRecorrido(lstPuntos, 500, 500, @"d:\borrar\fig2.jpg", false);
           string strGoogle= ObtenerGoogleCoordenadas(lstPuntos);
            ...
            while (dblT > dblTfin)
            {
                ...
                strLineas = QuitarUnaCruz(lstPuntos, ref dblCoste, ref lstRecorrido);
                if (strLineas != "")
            ...
            cImg.PintarRecorrido(lstPuntos, lstRecorrido, 500, 500, @"d:\borrar\fig3.jpg", false);
        }


        private string ObtenerGoogleCoordenadas(List<clsPunto> lstPuntos, double dblPorcentajeMargen = 0.05)
        {
            //Superior izquierdo: 40.457849100848, -3.7182743790764126
            //Inferior derecho: 40.434216380944434, -3.64930059642639
            double dblCajaSupIzq_Lat = 40.457849100848;
            double dblCajaSupIzq_Lon = -3.7182743790764126;
            double dblCajaInfDer_Lat = 40.434216380944434;
            double dblCajaInfDer_Lon = -3.64930059642639;
            // Recorre los puntos para obtener maximos y minimos
            double dblMinLon = double.MaxValue;
            double dblMinLat = double.MaxValue;
            double dblMaxLon = double.MinValue;
            double dblMaxLat = double.MinValue;
            foreach (clsPunto cPunto in lstPuntos)
            {
                double dblLon = cPunto.intX;
                double dblLat = cPunto.intY;
                if (dblLon > dblMaxLon)
                    dblMaxLon = dblLon;
                if (dblLon < dblMinLon)
                    dblMinLon = dblLon;
            ...
            // Introduce un margen por la izquierda, derecha, arriba y abajo
            double dblDif = dblMaxLat - dblMinLat;
            dblMaxLat = dblMaxLat + dblPorcentajeMargen * dblDif;
            dblMinLat = dblMinLat - dblPorcentajeMargen * dblDif;
            dblDif = dblMaxLon - dblMinLon;
            dblMaxLon = dblMaxLon + dblPorcentajeMargen * dblDif;
            dblMinLon = dblMinLon - dblPorcentajeMargen * dblDif;
            // Guarda las rutas
            List<clsRuta> lstRutas = new List<clsRuta>();
            Int32 intCuenta = 1;
            foreach (clsPunto cPunto in lstPuntos)
            {
                clsRuta cRuta = new clsRuta();
                cRuta.nombre = "P" + intCuenta;
                cRuta.orden = intCuenta;
                double dblPor = (cPunto.intX - dblMinLon) / (dblMaxLon - dblMinLon);
                cRuta.longitud = dblCajaSupIzq_Lon + dblPor * (dblCajaInfDer_Lon - dblCajaSupIzq_Lon);
                dblPor = (cPunto.intY - dblMinLat) / (dblMaxLat - dblMinLat);
                cRuta.latitud = dblCajaInfDer_Lat + dblPor * (dblCajaSupIzq_Lat - dblCajaInfDer_Lat);
                lstRutas.Add(cRuta);
                intCuenta++;
            }
            string output = JsonConvert.SerializeObject(lstRutas);
            return output;
        }
        ...
        public (double dblCosteSinCruces, Boolean blnHaMejorado) QuitarTodosLosCruces(ref List<clsPunto> lstPuntos)
        {
            Boolean blnHaMejorado = false;
            List<Int32> lstIndex = new List<int>();
            for (Int32 intI = 0; intI < lstPuntos.Count; intI++)
                lstIndex.Add(intI);
            HashSet<Int32> hsNoBorrar = new HashSet<int>();
            //B->E y C->F cambiarlo por B->C y E->F y darle la vuelta a E->...->C pasa a C->...->E
            for (Int32 intIndex = 0; intIndex < (lstIndex.Count); intIndex++)
            {
                Int32 intI = lstIndex[intIndex];
            ...
            double dblCosteSinCruces = CalcularDistanciaRecorrido(lstPuntos);
            return (dblCosteSinCruces, blnHaMejorado);
        }

        /// <summary>
        /// Calcula el recorrido a partir de una lista de puntos ordenados
        /// en el orden del recorrido.
        /// Calcula la distancia del recorrido cerrado (conecta el ultimo
        /// con el primero
        /// </summary>
        /// <param name="lstPuntos"></param>
        /// <returns></returns>
        private double CalcularDistanciaRecorrido(List<clsPunto> lstPuntos)
        {
            double dblCoste = 0;
